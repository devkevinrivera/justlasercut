...
const BannersPromocionales = () => {
    ...
    const fetchGallery = async () => {
        try {
            const fetchGalleryCall = await axios('/api/promotions');
    ...
    return (
        ...
        <Grid.Row>
            <Grid.Column width={12} verticalAlign="middle">
                <Header>CONFIGURACIÓN BANNERS PROMOCIONALES</Header>
            </Grid.Column>
            <Grid.Column width={4}>
...
const GalleryTable = ({ list = [], update, languageSelect }) => {
    ...
    const deleteItemGallery = async (id) => {
        try {
            const res = await axios.delete('/api/promotions', {
                params: {
                    id: id
                }
            });
...
const AddResource = ({ update, render, languageSelect }) => {
    ...
    const createResource = async () => {
        try {
            ...
            const createCall = await axios.post('/api/promotions', {
                title: seo,
                media: `${id}.${extension}`,
                link: href,
...
export default BannersPromocionales;